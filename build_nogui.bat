...
del noguiProjectX.jar

echo creating build subdirectory
mkdir build

echo compiling ProjectX with JAVA_HOME=%JAVA_HOME%
"%JAVA_HOME%\bin\javac.exe" -O -classpath lib\commons-net-1.3.0.jar -d build @noguisources.lst
...
"%JAVA_HOME%\bin\jar.exe" cfvm noguiProjectX.jar MANIFEST.MF -C build .
if errorlevel 1 goto ERROR
goto END
